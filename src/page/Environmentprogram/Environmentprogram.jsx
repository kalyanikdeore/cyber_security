...
export default function TechnologyDigitalPrograms({ onCTAClick }) {
  ...
  // Fetch all data using axios - UPDATED ENDPOINT
  const fetchAllData = async () => {
    try {
      setLoading(true);
      setError(null);

      // UPDATED: Changed from "/technology-programs" to "/environment-programs"
      const response = await axiosInstance.get("/environment-programs");
      ...
      if (response.data && Array.isArray(response.data)) {
        ...
        Array.isArray(response.data.initiatives)
      ) {
        // If response has nested initiatives property
        setPrograms(response.data.initiatives);
      } else if (
        response.data &&
        response.data.programs &&
        Array.isArray(response.data.programs)
      ) {
        // If response has nested programs property
      ...
      setError(
        `Failed to load programs: ${error.message}. Please try again later.`
      );
              ...
              <img
                ...
                onError={(e) => {
                  ...
                  e.target.parentElement.innerHTML = `
                    <div class="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#805B3A]/20 to-[#A1887F]/20">
                      <BookOpen class="w-16 h-16 text-[#805B3A]/40" />
                    </div>
                  `;
                }}
              />
                ...
                >
                  <img
                    ...
                    onError={(e) => {
                      e.target.style.display = "none";
                      e.target.parentElement.innerHTML = `
                        <div class="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#805B3A]/10 to-[#A1887F]/10">
                          <div class="text-center">
                            <BookOpen class="w-8 h-8 text-[#805B3A]/30" />
                            <p class="text-xs text-[#805B3A]/50 mt-1">Image ${
                              index + 2
                            }</p>
                          </div>
                        </div>
                ...
                <img
                  ...
                  onError={(e) => {
                    ...
                    e.target.parentElement.innerHTML = `
                      <div class="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#805B3A]/10 to-[#A1887F]/10">
                        <div class="text-center">
                          <BookOpen class="w-6 h-6 text-[#805B3A]/30" />
                          <p class="text-xs text-[#805B3A]/50 mt-1">${
                            index + 1
                          }</p>
                        </div>
                      </div>
              ...
              >
                <img
                  ...
                  onError={(e) => {
                    e.target.style.display = "none";
                    e.target.parentElement.innerHTML = `
                      <div class="h-32 sm:h-36 md:h-40 lg:h-44 w-full flex items-center justify-center bg-gradient-to-br from-[#805B3A]/10 to-[#A1887F]/10">
                        <div class="text-center">
                          <BookOpen class="w-8 h-8 text-[#805B3A]/30" />
                          <p class="text-xs text-[#805B3A]/50 mt-1">Slide ${
                            index + 1
                          }</p>
                        </div>
                      </div>
            ...
            >
              <img
                ...
                onError={(e) => {
                  ...
                  e.target.parentElement.innerHTML = `
                    <div class="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#805B3A]/20 to-[#A1887F]/20">
                      <BookOpen class="w-12 h-12 text-[#805B3A]/40" />
                    </div>
                  `;
                }}
              />
                  ...
                  <img
                    ...
                    onError={(e) => {
                      ...
                      e.target.parentElement.innerHTML = `
                        <div class="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#805B3A]/10 to-[#A1887F]/10">
                          <div class="text-center">
                            <BookOpen class="w-4 h-4 text-[#805B3A]/30" />
                            <p class="text-xs text-[#805B3A]/50 mt-1">${
                              index + 2
                            }</p>
                          </div>
    ...
    <section className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-12 sm:py-16 md:py-20 lg:py-24">
      ...
      >
        ...
        >
          <div className="inline-flex items-center justify-center gap-1 sm:gap-2 md:gap-3 mb-3 sm:mb-4">
            ...
            <span className="text-[#805B3A] font-semibold tracking-wider uppercase text-xs sm:text-sm">
              Environment Program Initiatives
            </span>
          ...
          <h2 className="text-xl sm:text-2xl md:text-3xl font-extrabold leading-tight text-slate-900 mb-3 sm:mb-4 px-2">
            ...
            <span className="bg-gradient-to-r from-[#805B3A] to-[#A1887F] bg-clip-text text-transparent">
              {" "}
              Action-Oriented Sustainability and Education
            </span>
          </h2>

          <p className="text-slate-600 max-w-3xl mx-auto text-xs sm:text-sm md:text-base px-2">
            Environmental degradation demands community-level action and
            education. Navanvesha instills a deep sense of environmental
            responsibility through participatory programs that yield immediate,
            visible, and long-term positive ecological change.
          </p>
        </motion.div>
        ...
        <div className="text-center mb-4 text-xs text-gray-500">
          Fetching from: {axiosInstance.defaults.baseURL}/environment-programs
        </div>
            ...
            {programs.length > 0 ? (
              ...
              <div className="text-center py-12">
                <p className="text-slate-600">
                  No programs available at the moment. Using fallback data.
                </p>
                <button
                  onClick={handleRetry}
